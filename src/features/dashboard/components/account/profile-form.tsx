...
import { usePhotoUpload } from '@/features/account/hooks/use-photo-upload';
import { useProfileForm } from '@/features/account/hooks/use-profile-form';
import { useConfirmEmail } from '@/features/account/hooks/use-confirm-email';
      ...
      <form onSubmit={form.handleSubmit((values) => saveProfile(values, photoUpload.photo))}>
        ...
        <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:justify-between">
          <div className='flex items-center justify-between sm:gap-4'>
            <DeleteAccountDialog />
            <Link className='font-inter font-medium text-base text-ds-text-plain' href={'/dashboard/account/change-password'}>{t('sidebar.password')}</Link>
          </div>
          <Button
            type="submit"
            loading={mutation.isPending}
            loadingText={t('actions.saving')}
            className="w-full sm:w-auto lg:w-57 text-base font-inter text-ds-text-inverse"
          >
            {t('actions.save')}
          </Button>
        </div>
